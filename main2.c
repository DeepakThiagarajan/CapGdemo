...
// int search_for_word(const char *path, const char *word, char *tokens[]) {
//     FILE *output_file;
//     char output_filename[MAX_PATH_LENGTH];
//     char grep_command[MAX_PATH_LENGTH + 15]; // 15 is the length of "grep -r '' > "
//     sprintf(output_filename, "%s/file.txt", path);
...
// Function to tokenize the contents of file.txt and display the tokens
void display_tokens() {
    char line[MAX_LINE_LENGTH];
    FILE *fp = fopen("filz.txt", "r");
